...
import { FloatButton, Drawer, Typography, Space, Spin, Button, Avatar, Layout, Tooltip, Modal, Input, Image } from 'antd';
import { 
  RobotOutlined, 
  CloseOutlined, 
  UserOutlined, 
...
import { Sender, Bubble, Conversations, Prompts } from '@ant-design/x';
import { queryGuideChat, GuideChatResponse } from './service';
import { useChatStorage, ChatMessage } from './useChatStorage';
...
const AiAssistant: React.FC = () => {
    ...
    if (activeSessionId) {
      ...
      if (msgs.length === 0) {
        ...
        const welcomeMsg: ChatMessage = {
          id: 'welcome',
          role: 'assistant',
          content: '您好！我是 SPD 助手，请问有什么可以帮您？例如：“SPD3.0 怎么收货”',
        };
  ...
  const handleSend = async (val: string) => {
    ...
    const botMsgLoading: ChatMessage = { id: `bot-${Date.now()}`, role: 'assistant', content: '', loading: true };
    ...
    if (currentSession && (currentSession.title === '新会话' || currentSession.title === '默认会话')) {
      updateSessionTitle(activeSessionId, text.substring(0, 10) + (text.length > 10 ? '...' : ''));
    }
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {res?.sources && res.sources.length > 0 && (
          <div style={{ marginTop: 8, background: '#f5f5f5', padding: '8px 12px', borderRadius: 8 }}>
            <Text type="secondary" strong style={{ fontSize: 12 }}>依据文档：</Text>
            {res.sources.map((src, idx) => (
              <div key={`src-${idx}`} style={{ marginTop: 4 }}>
                <Text style={{ fontSize: 12, color: '#1890ff' }}>{src.doc_name}</Text>
                <Paragraph 
        ...
        {res?.images && res.images.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
            {res.images.map((img, idx) => {
              ...
              return (
                <div key={`img-err-${idx}`} style={{ padding: 8, background: '#fffbe6', border: '1px solid #ffe58f', borderRadius: 8 }}>
    ...
    label: (
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', overflow: 'hidden' }}>
        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }} title={s.title}>
          {s.pinned && <PushpinFilled style={{ color: '#1890ff', marginRight: 4 }} />}
          {s.title}
        </span>
    ...
    <>
      ...
      >
        <Layout style={{ flex: 1, minHeight: 0, background: '#fff' }}>
          ...
          >
            <div style={{ padding: '16px 12px', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
              <Button type="primary" block icon={<PlusOutlined />} onClick={handleNewChat} style={{ marginBottom: 16 }}>
                新建会话
              </Button>
              
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <Text type="secondary" style={{ fontSize: 12 }}>快捷提示词</Text>
                <Button 
                  type="text" 
                  size="small" 
              ...
              <div style={{ maxHeight: 150, overflowY: 'auto', marginBottom: 16 }}>
                <Prompts
                  ...
                  styles={{
                    item: { padding: '8px 12px', background: '#f5f5f5', borderRadius: 8, cursor: 'pointer', marginBottom: 8 }
                  }}
                />
              </div>

              <Text type="secondary" style={{ fontSize: 12, marginBottom: 8 }}>历史会话</Text>
              <div style={{ flex: 1, overflowY: 'auto', marginLeft: -12, marginRight: -12 }}>
                <Conversations
          ...
          {/* 右侧聊天区 */}
          <Content style={{ display: 'flex', flexDirection: 'column', background: '#f9f9f9', height: '100%' }}>
                ...
                {currentMessages.map((msg, index) => (
                  <div key={`${msg.id}-${index}`} style={{ display: 'flex', flexDirection: 'column' }}>
                    <Bubble
                      ...
                      content={msg.role === 'user' ? msg.content : renderAssistantResponse(msg)}
                      avatar={msg.role === 'user' ? <Avatar icon={<UserOutlined />} style={{ background: '#87d068' }} /> : <Avatar icon={<RobotOutlined />} style={{ background: '#1890ff' }} />}
                      styles={{
                        content: {
